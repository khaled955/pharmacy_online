...
//     const hasResendDomain = !!process.env.RESEND_DOMAIN;

//     if (hasResendDomain) {
//       // PRODUCTION with domain — send real email
...
//         data: { email },
//         // Note: otp NOT returned in production with domain
//       });
//     }

//     // No domain yet — return OTP in response for testing
//     return NextResponse.json({
//       status: true,
//       message:
//         locale === "ar" ? "تم إنشاء رمز التحقق" : "OTP generated successfully",
//       data: { email, otp },
//     });
//   } catch (err: unknown) {
//     return NextResponse.json({
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Check if domain is configured for real email sending
    const hasResendDomain = !!process.env.RESEND_DOMAIN;

    if (hasResendDomain) {
      // Production with domain — send real email via Resend
      const { Resend } = await import("resend");
      const resend = new Resend(process.env.RESEND_API_KEY);
    ...
    // No domain yet — return OTP in response for testing
    // TODO: Remove otp from data after adding RESEND_DOMAIN
    return NextResponse.json({
      status: true,
      message:
        locale === "ar" ? "تم إنشاء رمز التحقق" : "OTP generated successfully",
